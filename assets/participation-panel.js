(function () {
  ...
  function createParticipationPanel(root, config = {}) {
    ...
    function renderAb(definition) {
      ...
      if (state.abSubmitted) return `<div class="participation-panel__question">${ab.question}</div><div class="participation-panel__stack">${ab.options.map((option, index) => `<div class="participation-panel__meter" data-meter style="--fill:${ab.fills[index] || 0}%"><span>${option.label} · <b>${ab.fills[index] || 0}%</b>${state.abSelection === option.id ? " ✓ 내 선택" : ""}</span></div>`).join("")}</div><div class="participation-panel__result"><span>${ab.participants}</span><span class="participation-panel__spacer"></span><button class="participation-panel__link" data-action="reset-ab" type="button">다시 선택</button></div>`;
    ...
    function render(animate = false) { const definition = getDefinition(); const enabledTabs = definition.tabs; if (!enabledTabs.includes(state.tab)) state.tab = definition.defaultTab; title.textContent = definition.title; subtitle.textContent = definition.subtitle; status.textContent = state.tab === "comments" ? String(state.comments.length) : state.tab === "poll" ? (state.pollSubmitted ? "✓ 응답 완료" : "● 미응답") : (state.abSubmitted ? "✓ 응답 완료" : "● 미응답"); tabs.forEach((tab) => { tab.hidden = !enabledTabs.includes(tab.dataset.tab); tab.classList.toggle("is-active", tab.dataset.tab === state.tab); }); content.innerHTML = state.tab === "poll" ? renderPoll(definition) : state.tab === "ab" ? renderAb(definition) : renderComments(definition); if (animate) setMeters(); }
