(function () {
  const stylesheetHref = `${new URL("participation-panel.css", document.currentScript?.src || window.location.href).href}?v=20260831-panel-r4`;
  const defaultConfig = {
    ...
    sections: {
      ...
      "data-book": {
        title: "Data Book",
        subtitle: "Data Book · 참여 도구",
        tabs: ["comments"],
        defaultTab: "comments",
        comments: { question: "나를 위한 자동차에 대한 생각을 남겨 주세요.", hint: "자동차 선택 경험이나 중요하게 보는 기준을 입력해 주세요.", placeholder: "의견을 남겨주세요…" }
      }
    }
  };
  ...
  function createParticipationPanel(host, config = {}) {
    ...
    const settings = { ...defaultConfig, ...config, user: { ...defaultConfig.user, ...(config.user || {}) }, sections: { ...defaultConfig.sections, ...(config.sections || {}) } };
    ...
    const getResponse = (sectionId = state.sectionId) => {
    ...
    function renderPoll(definition) {
      ...
      if (response.pollSubmitted) return `<div class="participation-panel__question">${poll.question}</div><div class="participation-panel__stack">${poll.options.map((option, index) => `<div class="participation-panel__meter" data-meter><i class="participation-panel__meter-fill" aria-hidden="true"><svg viewBox="0 0 100 1" preserveAspectRatio="none"><rect width="${fills[index]}" height="1"></rect></svg></i><span>${option} · <b>${fills[index]}%</b>${response.pollSelections.has(index) ? " ✓ 내 선택" : ""}</span></div>`).join("")}</div>${renderResultRow(poll.participants, "reset-poll")}`;
    ...
    function renderAb(definition) {
      ...
      if (response.abSubmitted) return `<div class="participation-panel__question">${ab.question}</div><div class="participation-panel__stack">${ab.options.map((option, index) => `<div class="participation-panel__meter" data-meter><i class="participation-panel__meter-fill" aria-hidden="true"><svg viewBox="0 0 100 1" preserveAspectRatio="none"><rect width="${fills[index]}" height="1"></rect></svg></i><span>${option.label} · <b>${fills[index]}%</b>${response.abSelection === option.id ? " ✓ 내 선택" : ""}</span></div>`).join("")}</div>${renderResultRow(ab.participants, "reset-ab")}<div class="participation-panel__completion-note">회차가 진행 중인 동안에는 언제든 바꿀 수 있습니다.</div>`;
