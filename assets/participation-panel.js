(function () {
  ...
  function createParticipationPanel(host, config = {}) {
    ...
    function renderPoll(definition) {
      ...
      if (response.pollSubmitted) return `<div class="participation-panel__question">${poll.question}</div><div class="participation-panel__stack">${poll.options.map((option, index) => `<div class="participation-panel__meter" data-meter><i class="participation-panel__meter-fill" aria-hidden="true" style="width: ${fills[index]}%;"></i><span>${option} · <b>${fills[index]}%</b>${response.pollSelections.has(index) ? " ✓ 내 선택" : ""}</span></div>`).join("")}</div>${renderResultRow(poll.participants, "reset-poll")}`;
    ...
    function renderAb(definition) {
      ...
      if (response.abSubmitted) return `<div class="participation-panel__question">${ab.question}</div><div class="participation-panel__stack">${ab.options.map((option, index) => `<div class="participation-panel__meter" data-meter><i class="participation-panel__meter-fill" aria-hidden="true" style="width: ${fills[index]}%;"></i><span>${option.label} · <b>${fills[index]}%</b>${response.abSelection === option.id ? " ✓ 내 선택" : ""}</span></div>`).join("")}</div>${renderResultRow(ab.participants, "reset-ab")}<div class="participation-panel__completion-note">회차가 진행 중인 동안에는 언제든 바꿀 수 있습니다.</div>`;
